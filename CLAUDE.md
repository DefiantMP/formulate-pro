...
- `formulate-pro-engine/` (moved into `lib/calc-engine/`) is the tested calculation engine. Do not rewrite its math logic without explicit instruction. Passing test suite, 54 tests at last check across `lib/calc-engine/tests/`, `lib/arithmetic.test.ts`, and `lib/aiVerification.test.ts` (`npm test`) — run before and after any change that touches calculation or AI-verification code.
