...
- `formulate-pro-engine/` (moved into `lib/calc-engine/`) is the tested calculation engine. Do not rewrite its math logic without explicit instruction. Passing test suite, 62 tests at last check across `lib/calc-engine/tests/`, `lib/arithmetic.test.ts`, and `lib/aiVerification.test.ts` (`npm test`) — run before and after any change that touches calculation or AI-verification code.
...
- [ ] Regrind mode validation pass — pending a real regrind formulation sheet, now ideally multi-lot since multi-lot blending shipped 2026-07-14, and ideally covering solve-for-target-tablet-count mode too since that shipped 2026-07-16
